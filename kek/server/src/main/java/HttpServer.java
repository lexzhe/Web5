...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        File file = new File(root, request.getUriUTF8());
        if (file.exists()) {
            if (file.isDirectory()) {
    ...
    private String getETag(byte[] kek) {
        long resp = Arrays.hashCode(kek);
        return Long.toString(resp);
    }
...
//        }
        ...
        private String getUriUTF8() throws UnsupportedEncodingException {
            try {
